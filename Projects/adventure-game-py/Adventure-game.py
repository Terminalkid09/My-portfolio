...
def explore_area(key):
    ...
    # Boss fight
    print(f"\n🔥 A powerful presence approaches in the {area['name']}...")
    boss = area["boss"]
    boss_result = combat(boss, is_boss=True)
    if boss_result:
        print(f"\n🏆 You have cleared the {area['name']}!")
...
def random_npc_event(area_key):
    if random.random() < 0.3:
        ...
        print(f"\n✨ You encounter a {npc} on your journey.")
        print(f"{npc}:")
        if npc == "Lost Explorer":
            print("'I’ve been wandering here for days... take this, it might help you.'")
            player["inventory"].append("Potion")
